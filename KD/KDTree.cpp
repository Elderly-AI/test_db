...
KDTree::KDTree(const std::string &id, const DBManager *db_manager):BaseDataStructure(id, db_manager){
	root = new KDTreeNode(std::vector<std::string>{}, db_manager);
}
...
void KDTree::add(const std::vector<double>& key, const std::string& data){
	...
	while(((*currient_kd_tree_node)->left_node != nullptr) && ((*currient_kd_tree_node)->right_node != nullptr)){

	//while((*currient_kd_tree_node != nullptr)&&((*currient_kd_tree_node)->containers.size() == 1)){
		//std::cout << "hi " << (*currient_kd_tree_node)->containers.size() << "\n";
		if(Cmp(depth)(load_key((*currient_kd_tree_node)->containers[0]), key)){
...
std::vector<std::string> KDTree::range_search(const std::vector<double>& min, const std::vector<double>& max){
	...
	while(!kd_tree_node_queue.empty()){
		...
		std::cout << "compare ";
		for(auto &e : load_key(currient_kd_tree_node->containers[0])){
			std::cout << e << " ";
		}
		std::cout << "depth " << currient_kd_tree_depth << "\n";
		...
		if(Cmp(currient_kd_tree_depth)(load_key(currient_kd_tree_node->containers[0]), min)){
			...
			if(currient_kd_tree_node->containers.size() == 1){
				//std::cout << "answer " << load_key(currient_kd_tree_node->containers[0])[currient_kd_tree_depth]<< "\n";
				//std::cout << "currient_kd_tree_depth " << currient_kd_tree_depth << "\n";
				out_data.push_back(currient_kd_tree_node->containers[0]);

				auto is_vector_in_range = std::find_if_not(load_key(currient_kd_tree_node->containers[0]).begin(), 
					load_key(currient_kd_tree_node->containers[0]).end(), [](int i){
						return i % 2;
					});
			}
			else{
				//std::cout << "long\n";
				//TODO
				auto left_bound = std::upper_bound(currient_kd_tree_node->containers.begin(), currient_kd_tree_node->containers.end(), min, 
					[currient_kd_tree_depth, this](std::vector<double> min, std::string key){
						return (!Cmp(currient_kd_tree_depth)(load_key(key), min));
					}
				);

				//std::cout << "min " << min[currient_kd_tree_depth] << "\n";
				//std::cout << "left_bound " << load_key(*left_bound)[currient_kd_tree_depth] << "\n";

				auto right_bound = std::lower_bound(currient_kd_tree_node->containers.begin(), currient_kd_tree_node->containers.end(), max, 
					[currient_kd_tree_depth, this](std::string key, std::vector<double> max){
						return (!Cmp(currient_kd_tree_depth)(max, load_key(key)));
					}
				);

				//std::cout << "max " << max[currient_kd_tree_depth] << "\n";
				//std::cout << "right_bound " << load_key(*right_bound)[currient_kd_tree_depth] << "\n";
				//for(auto &e : currient_kd_tree_node->containers){
				//	std::cout << "answer " << load_key(e)[currient_kd_tree_depth] << "\n";
				//}

				out_data.insert(out_data.end(), left_bound, right_bound);
			}
